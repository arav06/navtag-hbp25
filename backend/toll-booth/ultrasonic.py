...
import sys
# ===== Configuration =====
MIN_DISTANCE_CM = 50        # Alert distance threshold
KNOWN_WIDTH = 20.0          # Real-world width of target object (cm)
FOCAL_LENGTH = 500          # Calibrate this value for your camera
MIN_CONTOUR_AREA = 1000     # Increased: Minimum area to consider as valid detection
MERGE_DISTANCE = 50         # Pixel distance to merge nearby detections
MOTION_FRAMES = 10          # Increased: Consecutive frames with movement required
SHUTDOWN_DELAY = 5          # Seconds to wait after confirmed movement
API_ENDPOINT = f"http://{sys.argv[1]}:5002/capture"  # Replace with your endpoint
...
def is_moving(current_centers, previous_centers, threshold=20):  # Increased threshold
...
fgbg = cv2.createBackgroundSubtractorMOG2(history=500, varThreshold=32, detectShadows=False)  # Increased varThreshold
...
while True:
    ...
    thresh = cv2.threshold(fgmask, 25, 255, cv2.THRESH_BINARY)[1]  # Increased threshold
    ...
    # Movement detection and API trigger
