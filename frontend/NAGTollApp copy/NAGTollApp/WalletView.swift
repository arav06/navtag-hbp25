...
struct WalletView: View {
    ...
    private let balanceUpdatePublisher = Timer.publish(every: 30, on: .main, in: .common).autoconnect()  // Timer to fetch balance every 30 seconds
    ...
    var body: some View {
        NavigationView {
            ...
            .onReceive(balanceUpdatePublisher) { _ in
                fetchBalance()  // Continuously fetch balance every 30 seconds
            }
        }
    }
                ...
                if let jsonResponse = try JSONSerialization.jsonObject(with: data) as? [String: Any],
                   let balance = jsonResponse["balance"] as? Double {
                    DispatchQueue.main.async {
                        navTagBalance = balance  // Directly display server balance
                        onComplete?()
                    }
                }
            } catch {
